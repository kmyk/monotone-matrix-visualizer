function generateMatrixByHillClimbing(h: number, w: number, range: number, evaluate: (f: number[][]) => number): number[][] {
...
function estimateHowMuchNotMonge(f: number[][]): number {
    const h = f.length;
    const w = f[0].length;
...
function generateMonge(h: number, w: number, range: number): number[][] {
    return generateMatrixByHillClimbing(h, w, range, estimateHowMuchNotMonge);
}

function estimateHowMuchNotTotallyMonotone(f: number[][]): number {
    const h = f.length;
    const w = f[0].length;
...
function generateTotallyMonotone(h: number, w: number, range: number): number[][] {
    return generateMatrixByHillClimbing(h, w, range, estimateHowMuchNotTotallyMonotone);
}

function estimateHowMuchNotMonotone(f: number[][]): number {
    const h = f.length;
    const w = f[0].length;
...
function generateMonotone(h: number, w: number, range: number): number[][] {
    return generateMatrixByHillClimbing(h, w, range, estimateHowMuchNotMonotone);
}

function generateRandom(h: number, w: number, range: number): number[][] {
    const f = [];
    for (let y = 0; y < h; ++ y) {
        f.push([]);
        for (let x = 0; x < w; ++ x) {
...
function rgbFromCMYK(c: [number, number, number, number]): [number, number, number] {
    const k = c[3];
    return [
...
function cmykFromRGB(c: [number, number, number]): [number, number, number, number] {
...
function getColorOfValue(value: number, range: number): string {
...
function visualizeMatrix(f: number[][], range: number, table: HTMLTableElement): void {
    const h = f.length;
    const w = f[0].length;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const w = 18;
    const range = 100;
    visualizeMatrix(generateRandom(h, w, range), range, document.getElementById('random') as HTMLTableElement);
    visualizeMatrix(generateMonotone(h, w, range), range, document.getElementById('monotone') as HTMLTableElement);
    visualizeMatrix(generateTotallyMonotone(h, w, range), range, document.getElementById('totally-monotone') as HTMLTableElement);
    visualizeMatrix(generateMonge(h, w, range), range, document.getElementById('monge') as HTMLTableElement);
});
